...
def create_chatbot_router(appointment_controller=None) -> APIRouter:
    """
    Factory function to create chatbot router with external dependencies.

    Args:
        appointment_controller: Optional appointment controller. If None, will be imported from host project.

    Returns:
        Configured APIRouter with chatbot endpoints.
    """
    if not appointment_controller:
        try:
            from core.controllers.appointment_controller import AppointmentController
            appointment_controller = AppointmentController()
        except ImportError:
            raise RuntimeError("Appointment controller not found. Please provide it as an argument.")
