...
class AppointmentCRUD:
    ...
    async def create_appointment(self, appointment: dict) -> Appointment:
        try:
            ...
            return await self.engine.save(Appointment(**appointment))
    ...
    async def get_appointments_by_date(
        ...
        try:
            logging.info(f"Finding appointments for date: {appointment_date}")
            dt = _date_to_datetime(appointment_date)
    ...
    async def get_patient_appointments(
        ...
        try:
            logging.info(f"Finding appointments for patient: {patient_id}")
            return await self.engine.find(
                Appointment,
                Appointment.patient_id == patient_id,
            )
        except Exception as error:
            logging.error(f"Error finding patient appointments: {error}")
            raise